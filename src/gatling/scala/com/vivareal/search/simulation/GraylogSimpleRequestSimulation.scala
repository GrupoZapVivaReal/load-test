...
class GraylogSimpleRequestSimulation extends Simulation {
  ...
  val scn = scenario("GraylogSimpleRequestSimulation").during(maxDuration seconds) {
    feed(feeder)
      .exec(http("GraylogSimpleRequests")
      .get("${" + uriField + "}"))
  }
